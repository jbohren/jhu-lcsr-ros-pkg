...
robManipulator_AAB::robManipulator_AAB( const vctFrame4x4<double>& Rtw0 )
{  this->Rtw0 = Rtw0;  }

robManipulator_AAB::robManipulator_AAB( const std::string& linkfile, 
				const vctFrame4x4<double>& Rtw0 ){
  
  this->Rtw0 = Rtw0;
  
  if( LoadRobot( linkfile ) == robManipulator_AAB::EFAILURE ){
...
void robManipulator_AAB::Attach( robManipulator_AAB* tool )
{ tools.push_back( tool ); }
				
robManipulator_AAB::Errno robManipulator_AAB::LoadRobot( const std::string& filename ){

  if( filename.empty() ){
    CMN_LOG_RUN_ERROR << CMN_LOG_DETAILS
		      << " No configuration file!."
		      << std::endl;
    return robManipulator_AAB::EFAILURE;
  }
  ...
  return robManipulator_AAB::ESUCCESS;;
}
...
vctFrame4x4<double> 
robManipulator_AAB::ForwardKinematics( const vctDynamicVector<double>& q,
				   int N ) const {
...
robManipulator_AAB::InverseKinematics( vctDynamicVector<double>& q, 
				   const vctFrm3& Rts,
				   double tolerance, 
				   size_t Niterations ){
  vctFrm3 Rtsn( Rts );
...
robManipulator_AAB::InverseKinematics( vctDynamicVector<double>& q, 
  ...
  if( i==Niterations ) return robManipulator_AAB::EFAILURE;
  else return robManipulator_AAB::ESUCCESS;;
}
 ...
 */
void robManipulator_AAB::JacobianBody( const vctDynamicVector<double>& q ) const {
...
void robManipulator_AAB::JacobianSpatial( const vctDynamicVector<double>& q ) const{
...
robManipulator_AAB::RNE( const vctDynamicVector<double>& q,
...
vctDynamicVector<double> 
robManipulator_AAB::CCG( const vctDynamicVector<double>& q,
...
robManipulator_AAB::BiasAcceleration( const vctDynamicVector<double>& q,
				  const vctDynamicVector<double>& qd ) const {
...
robManipulator_AAB::JSinertia( const vctDynamicVector<double>& q ) const {
...
void robManipulator_AAB::JSinertia( double **A,
				const vctDynamicVector<double>& q ) const {

  if( q.size() != links.size() ){
    std::cerr << "robManipulator_AAB::JSinertia: Expected " << links.size()
	      << " values. Got " << q.size()
	      << std::endl;
    return;
  }
...
// Ac is column major!
void robManipulator_AAB::OSinertia( double Ac[6][6], 
				const vctDynamicVector<double>& q ) const {
...
vctDynamicVector<double> 
robManipulator_AAB::InverseDynamics(const vctDynamicVector<double>& q,
				const vctDynamicVector<double>& qd,
...
robManipulator_AAB::SE3Difference( const vctFrame4x4<double>& Rt1,
...
void 
robManipulator_AAB::AddIdentificationColumn
...
vctDynamicMatrix<double> 
robManipulator_AAB::JacobianKinematicsIdentification
...
void robManipulator_AAB::PrintKinematics( std::ostream& os ) const {
